...
import { ensureValidNumber } from "./data-utils.js"

export function streamSlide(entity, payload) {
  if (!Array.isArray(entity.data) || !entity.data.length) return

  const nextIndex =
    ensureValidNumber(entity.streamIndex, entity.data.length - 1) + 1
  const nextValue = ensureValidNumber(payload?.value, 0)
