...
function realtimeConfigGet(entity) {
  ...
  const variation = resolveVariation(entity, bounds)
  const currentValue = resolveCurrentValue(entity, bounds)
...
function streamSeedBuild(entity, realtime) {
  ...
  const baseValue = realtime.currentValue
...
function resolveBounds(entity) {
  const realtimeMin = entity?.realtime?.min
  const realtimeMax = entity?.realtime?.max
  const entityMin = entity?.min
  const entityMax = entity?.max

  const min = Number.isFinite(realtimeMin)
    ? realtimeMin
    : Number.isFinite(entityMin)
      ? entityMin
      : 0
  const max = Number.isFinite(realtimeMax)
    ? realtimeMax
    : Number.isFinite(entityMax)
      ? entityMax
      : 100
...
function resolveVariation(entity, bounds) {
  const realtimeVariation = entity?.realtime?.variation
  const entityVariation = entity?.variation

  if (Number.isFinite(realtimeVariation) && realtimeVariation > 0) {
    return realtimeVariation
  }
  if (Number.isFinite(entityVariation) && entityVariation > 0) {
    return entityVariation
  }

  const range = Math.max(1, bounds.max - bounds.min)
  return Math.max(1, Math.round(range * 0.1))
}

function resolveCurrentValue(entity, bounds) {
  const realtimeCurrentValue = entity?.realtime?.currentValue
  if (Number.isFinite(realtimeCurrentValue)) return realtimeCurrentValue

  const yKey = entity?.streamYKey ?? "value"
  const lastRow = Array.isArray(entity?.data) ? entity.data.at(-1) : null
  const lastValue = lastRow?.[yKey]
  if (Number.isFinite(lastValue)) return lastValue
