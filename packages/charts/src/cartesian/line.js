...
export const line = {
  ...
  renderLineChart(entityId, children, api, config = {}) {
    ...
    const width = parseDimension(config.width || style.width) || parseDimension(entity.width) || 800
    const height = parseDimension(config.height || style.height) || parseDimension(entity.height) || 400
  ...
  _getAllNumericValues(entity) {
    ...
    entity.data.forEach(d => {
      Object.keys(d).forEach(key => {
        if (key !== 'name' && key !== 'x' && key !== 'date' && typeof d[key] === 'number') {
  ...
  _createSharedContext(entity, entityId) {
    ...
    const storedDimensions = entityId ? compositionDimensions.get(entityId) : null
  ...
  renderXAxis(config, entityId, api) {
    ...
    const labels = entity.data.map((d, i) => d[dataKey] || d.name || d.x || d.date || String(i))
