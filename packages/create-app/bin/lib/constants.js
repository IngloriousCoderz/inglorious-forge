export const CAPACITOR_VERSION = "^8.5.0"
export const VITE_PLUGIN_PWA_VERSION = "^1.3.0"
export const VITE_PWA_ASSETS_GENERATOR_VERSION = "^1.0.0"
export const WORKBOX_WINDOW_VERSION = "^7.4.1"

export const INDENTATION = 2
export const AFTER_FIRST_CHARACTER_INDEX = 1
export const EMPTY_LENGTH = 0
export const FIRST_CHARACTER_INDEX = 0
export const FIRST_ITEM_INDEX = 0
export const NPM_VIEW_TIMEOUT_MS = 10_000
export const USER_ARG_START_INDEX = 2
export const VITE_APP_TEMPLATES = new Set(["js", "ts"])
